...
>
> **Chromium note:** the `+rkmpp` Chromium shipped on Pi Desktop decodes through libv4l-rkmpp, not through this driver; see KI-6. Firefox and mpv use this driver.
...
`RKVA_RGA_P010=0` forces the CPU repack for 10-bit. The 8-bit NV12 copy stays on the CPU by default (as in 2.1.5) until the RGA path is fenced against the compositor's reads; `RKVA_RGA_COPY=1` enables it, `RKVA_RGA_COPY=0` turns every RGA copy off for A/B tests. The 8-bit NV12 copy has used RGA
